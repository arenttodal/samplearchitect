...
function generateKSP(samples, stats, config, positions, customUiHeight) {
  ...
  // Layout: use builder positions if available, otherwise fallback to 4×2 grid
  ...
  var uiHeight = customUiHeight || (numRows <= 1 ? 320 : 430);
  ...
  enabledControls.forEach(function(item, index) {
    ...
    // Use builder position if available, otherwise fallback to grid
    var pos = hasPositions && positions.find(function(p) { return p.key === key; });
    var x, y, labelX, labelY;
    if (pos) {
      x = pos.x;
      y = pos.y;
      labelX = pos.labelX;
